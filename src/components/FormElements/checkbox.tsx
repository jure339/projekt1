...
export function Checkbox({ withIcon, label, name, withBg, minimal, onChange, radius }: PropsType) {
  const id = useId();
    ...
    <div>
      ...
      >
        <div className="relative">
          <input type="checkbox" onChange={onChange} name={name} id={id} className="peer sr-only" />
          ...
          >
            {!withIcon && <span className="hidden size-2.5 rounded-sm bg-primary" />}

            {withIcon === 'check' && <CheckIcon className="hidden text-primary" />}
