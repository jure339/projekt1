...
export default function TrainingsList({ className }: { className?: string }) {
  ...
  async function load(currentTeamId: string) {
    ...
    try {
      const res = await fetch(`/api/treningi?ekipaId=${encodeURIComponent(currentTeamId)}`, {
        cache: 'no-store',
      });
  ...
  return (
    <div className={cn(container, className)}>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-body-2xlg font-bold text-dark dark:text-white">Trainings</h2>
        ...
        <TableBody>
          {loading ? (
            ...
            rows.map((t) => (
              ...
              >
                <TableCell className="!text-left">{new Date(t.zacetek).toLocaleString()}</TableCell>
                <TableCell className="!text-left">{new Date(t.konec).toLocaleString()}</TableCell>
